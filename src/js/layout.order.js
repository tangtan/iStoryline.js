...
function storyOrder(data, orderInfo) {
  CharacterOrder = orderInfo;
  storydata = data;
  //entity keyTimeframe maxTimeframeTable
  preprocessData(data);
  sortLocationTree(data.locationTree);
  // let errOrderLocation=[];
  // // let exchangeMap=[];//这个location到底在哪里放着
  // // for (let time=data.keyTimeframe[0];time<=data.keyTimeframe[data.keyTimeframe.length-1];time++) exchangeMap[time]=new Map();
  // for (let [firstCha,lastCha] of CharacterOrder){

  //     for (let time=0;time<=data.keyTimeframe[data.keyTimeframe.length-1];time++){
  //         let firstSession=cha2Session(firstCha,time);
  //         let lastSession=cha2Session(lastCha,time);

  //         if (firstSession!==-1&&lastSession!==-1&&lastSession!==firstSession){//(exchangeMap[time].get(session2Loction(lastSession))||session2Loction(lastSession))<(exchangeMap[time].get(session2Loction(firstSession))||session2Loction(firstSession))){
  //             let Location1=session2Loction(firstSession);
  //             let Location2=session2Loction(lastSession);
  //             errOrderLocation.push([time,Location1,Location2]);
  //         }
  //     }
  ...
  //         });
  //         return locationTree.indexOf(ans);
  //     }
  //     // noinspection JSAnnotator
  ...
  //                 if (character.entity===cha&&time>=character.start&&time<character.end){
  //                     return SessionId;
  //                 }
  //             }
  //         }
...
//           betweenSessionPair = new Map(),
//           betweenLocationPair = new Map();
//         if (!(timeframe[0] >= beginTime && timeframe[0] < endTime)) continue;
//         //接下来分类讨论到底是Location不同还是Session之间还是session之内
//         //交换Location顺序，交换Session顺序，交换sessionTable内的顺序
//         //光交换不行，把东西存到三个数组，然后排序才行。
//         let firstSessionId = cha2Session(firstCha, timeframe[0]);
...
//             let sessionOrder = inSessionPair.get(session);
//             if (sessionOrder === undefined)
//               inSessionPair.set(session, [[firstOrder, lastOrder]]);
//             else
//               inSessionPair.set(
//                 session,
//                 sessionOrder.push([firstOrder, lastOrder])
//               );
//           }
//           continue;
//         }
...
//           let sessionsOrder = betweenSessionPair.get(sessions);
//           if (sessionsOrder === undefined)
//             sessionsOrder.set(sessions, [[indexofFirst, indexofLast]]);
//           else
//             sessionsOrder.set(
//               sessions,
//               sessionsOrder.push([indexofFirst, indexofLast])
//             );
//           continue;
//         } else {
//           let locations = timeframe[1].children;
...
//             let LocationOrder = betweenLocationPair.get(locations);
//             if (LocationOrder === undefined)
//               betweenLocationPair.set(locations, [[firstOrder, lastOrder]]);
//             else
//               betweenLocationPair.set(
//                 locations,
//                 LocationOrder.push([firstOrder, lastOrder])
//               );
//           }
//         }
//       }
...
//   });
  ...
  // sequence.forEach(timeframe=>{
  //     let Locations=timeframe[1].children;
  //     let timeframeBegin=timeframe[0];
  //     for (let i=0;i<Locations.length;i++){
  //         Locations[i].locationOrder=i;
  //     }
  //     let thistimeErrorder=errOrderLocation.filter(pair=>pair[0]===timeframeBegin);
  //     Locations.sort((a,b)=>{
  //         for (let [_,first,last] of thistimeErrorder){
  //             if (a.locationOrder===first&&b.locationOrder===last) return -1;
  //             if (b.locationOrder===first&&a.locationOrder===last) return 1;
  //         }
  //         return 0;
  //     })
  // });
