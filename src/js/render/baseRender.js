function _checkAngle(storyline, storylineID, segmentID, storyNodeID, nxtTime, STDANGLE = 0.271975) {
...
function _checkMoveRules(storyline, storylineID, segmentID, storyNodeID) {
    ...
    if (_checkAngle(storyline, storylineID, segmentID, nowID, nowID + 1)) return true;
    return false;
}
...
{
    let nxtK = k + 1;
    while (nxtK < tmpSketchNodes.length && tmpSketchNodes[k][1] === tmpSketchNodes[nxtK][1]) nxtK++;
    nxtK--;
    return nxtK;
}
    ...
    if (stylish !== undefined || relate !== undefined) {
        if (stylish !== undefined) {
            for (let i = 0; i < stylish.length; i++) {
                ...
                tmpDivideMarks[storylineID][divideCnts[storylineID]] = [stylish[i][1], stylish[i][3]];
                divideCnts[storylineID]++;
                tmpDivideMarks[storylineID][divideCnts[storylineID]] = new Array();
                tmpDivideMarks[storylineID][divideCnts[storylineID]] = [stylish[i][2], 'Normal'];
                divideCnts[storylineID]++;
            }
        }
        if (relate != undefined) {
            for (let i = 0; i < relate.length; i++) {
                ...
                for (let j = 0; j < relate[i][0].length; j++) {
                    ...
                    tmpDivideMarks[storylineID][divideCnts[storylineID]] = new Array();
                    tmpDivideMarks[storylineID][divideCnts[storylineID]] = [relate[i][1], relate[i][3] + '_' + String(i) + '_' + String(num)];
                    divideCnts[storylineID]++;
                    tmpDivideMarks[storylineID][divideCnts[storylineID]] = new Array();
                    tmpDivideMarks[storylineID][divideCnts[storylineID]][0] = [relate[i][2], 'Normal'];
                    divideCnts[storylineID]++;
                }
            }
        }
...
export function extent(storyline, retStoryline,SCALE = 20, SCALELENGTH = 200000, STEPLENGTH = 20) {
    let maxLength = 0;
    for (let i = 0; i < storyline.length; i++) {
...
export function calculateOriginNodes(initialNodes, timeframeTable, entities, TIMEFRAMESPAN = 50) {
    ...
    for (let i = 0; i < entities.length; i++) {
        ...
        for (let j = 0; j < _storySegment.length; j++) {
            ...
            while (cnt < initialNodes[i].length && _checkInSameSegment(_storySegment[j], Math.floor(initialNodes[i][cnt][0] / TIMEFRAMESPAN))) {
                originNodes[i][j][tot] = new Array();
                originNodes[i][j][tot] = [initialNodes[i][cnt][0], initialNodes[i][cnt][1]];
                cnt++;
                tot++;
            }
        }
        ...
        for (let j = 0; j < tmpSmoothNodes[i].length; j++) {
            ...
            for (let k = 0; k < tmpSmoothNodes[i][j].length; k++) {
                ...
                while (flagSplit === 1 || flagSplit === 2) {
                    if (flagSplit === 1) {
                        if (cntNodes !== 0) {
                            splitNodes[i][cntSegments][cntNodes] = deepCopy(tmpSmoothNodes[i][j][k]);
                            ++cntSegments;
                            cntNodes = 0;
                            if (k < tmpSmoothNodes[i][j].length - 1) splitNodes[i][cntSegments] = new Array();
                        }
...
export function initializeSplitMarks(storyline, characterName, relateInfo, stylishInfo) {
...
export function linkNodes(nodes, type = 0, SAMPLERATE = 20, style = 0)
{
    ...
    if(style === 0){
        ...
        if(type == 0){ //拐角中线平滑
            p[0][0] = nodes[0][0];p[0][1] = nodes[0][1];
            p[1][0] = (nodes[0][0] + nodes[1][0]) * 0.5;p[1][1] = nodes[0][1];
            p[2][0] = (nodes[0][0] + nodes[1][0]) * 0.5;p[2][1] = nodes[1][1];
            p[3][0] = nodes[1][0];p[3][1] = nodes[1][1];
        }
        else{
            for(let i = 0;i < 4;i ++){
                p[i][0] = nodes[i][0];
    ...
    for (let k = 0; k < tmpSketchNodes.length - 1; k++) {
        if (tmpSketchNodes[k][1] !== tmpSketchNodes[k + 1][1]) {
            ...
            for (let z = 0; z <= SAMPLERATE; z++) {
                ...
                styledNodes[cnt][0] = tmpSketchNodes[k][0] + tmpLength * z / SAMPLERATE;
                styledNodes[cnt][1] = Math.sin(z * WAVERATE * PI / SAMPLERATE) * WAVEHEIGHT + tmpSketchNodes[k][1];
                cnt++;
            }
            k = nxtK;
        }
...
function _styleZigzag(tmpSketchNodes, ZIGHEIGHT = 500)
{
    ...
    for (let k = 0; k < tmpSketchNodes.length - 1; k++) {
        if (tmpSketchNodes[k][1] !== tmpSketchNodes[k + 1][1]) {
            ...
            for (let z = 0; z < SAMPLERATE; z++) {
                styledNodes[cnt] = new Array();
                styledNodes[cnt][0] = tmpSketchNodes[k][0] + tmpLength * (z + 0.5) / SAMPLERATE;
...
{
    const{aNodes,bNodes} = _forRelateTurn(tmpSketchNodes,0.2,stdY,((cntNum & 1) ? -1 : 1) * COLLIDEHEIGHT);
    let aimNodes = new Array();
    let cnt = 0;
...
function _relateKnot(tmpSketchNodes,stdY,KNOTHEIGHT = 2000)
{
    ...
    let aimNodes = new Array();
    let midLength = 0.2 * (tmpSketchNodes[tmpSketchNodes.length -1][0] - tmpSketchNodes[0][0]);
    let cnt = 0;
    ...
    for (let i = 0; i <= SAMPLERATE; i ++) {
        ...
        aimNodes[cnt][1] = Math.random() * KNOTHEIGHT * (Math.random > 0.5 ? 1 : -1) + stdY;
        cnt++;
    }
...
function _relateTwine(tmpSketchNodes,TWINEHEIGHT = 500,PI = 3.14)
{
    ...
    let midLength = (tmpSketchNodes[tmpSketchNodes.length -1][0] - tmpSketchNodes[0][0]) * 0.6;
    ...
    for (let i = 0; i <= SAMPLERATE; i++) {
        ...
        aimNodes[cnt][1] = ((cntNum & 1) ? -1 : 1) * (Math.sin(i * WAVERATE * PI / SAMPLERATE)) * TWINEHEIGHT + stdY;
        cnt++;
    }
    for (let i = 0;i < bNodes.length;i ++) {
        aimNodes[cnt ++] = deepCopy(bNodes[i]);
...
function _forRelateTurn(tmpSketchNodes, rate, stdY, HEIGHT)
{
    ...
    if (!(SAMPLERATE & 1)) SAMPLERATE += 1;
    let aNodes = linkNodes([tmpSketchNodes[0],[[staX,stdY + HEIGHT]]],0,SAMPLERATE);
    let bNodes = linkNodes([[endX,stdY + HEIGHT],tmpSketchNodes[tail]],0,SAMPLERATE);
