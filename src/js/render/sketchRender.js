...
function calculateSketchNodes(renderNodes, originNodes, group, splitMarks, groupPosition, initialGraph, relate, stylish) {
    ...
    for (let i = 0; i < tmpSketchNodes.length; i++) {
        ...
        for (let j = 0; j < tmpSketchNodes[i].length; j++) {
            ...
            for (let k = 0; k < tmpSketchNodes[i][j].length - 1; k++) {
                if (tmpSketchNodes[i][j][k][0] === tmpSketchNodes[i][j][k + 1][0]) {
                    ...
                    if (tmpSketchNodes[i][j][k][1] !== tmpSketchNodes[i][j][k + 1][1]) {
                        ctrl[0][1] = 0;
                        let SAMPLERATE = Math.floor(_getLength(tmpSketchNodes[i][j][k], tmpSketchNodes[i][j][k + 1]) / 1000);
                        if (!(SAMPLERATE & 1)) SAMPLERATE += 1;
                        tmpAimNodes = linkNodes([tmpSketchNodes[i][j][k], tmpSketchNodes[i][j][k + 1]], 0, SAMPLERATE);
                    }
                    else {
                        ctrl[0][1] = 0;
                        tmpAimNodes = [deepCopy(tmpSketchNodes[i][j][k]), deepCopy(tmpSketchNodes[i][j][k + 1])];
                    }
                    for (let z = 0; z < tmpAimNodes.length - 1; z++) {
                        let SAMPLERATE = Math.floor(_getLength(tmpAimNodes[z], tmpAimNodes[z + 1]) / 1000);
                        ...
                        let aimNodes = linkNodes([tmpAimNodes[z], ctrl[0], ctrl[1], tmpAimNodes[z + 1]], 1, SAMPLERATE);
            ...
            sketchNodes[i][j][cnt++] = deepCopy(tmpSketchNodes[i][j][tmpSketchNodes[i][j].length - 1]);
        }
    }
...
function shake(p, a, b, type = 1, distanceRate = 0.25, SHAKEY = 300, SHAKEX = 0.5) {
    if (a[1] === b[1]) {
        let length = b[0] - a[0];
